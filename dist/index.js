...
var collectCopyTargets = async (root, targets, flatten) => {
  ...
  for (const { src, dest, rename } of targets) {
    ...
    for (const matchedPath of matchedPaths) {
      ...
      copyTargets.push({
        src: matchedPath,
        dest: path.join(destDir, rename != null ? rename : base)
      });
    }
  }
  return copyTargets;
...
var copyAll = async (rootSrc, rootDest, targets, flatten) => {
  ...
  await Promise.all(copyTargets.map(({ src, dest }) => fs.copy(src, path.join(rootDest, dest))));
...
var servePlugin = ({
  ...
  return {
    ...
    configureServer({ httpServer, middlewares, ws }) {
      ...
      watcher = chokidar.watch(targets.flatMap((target) => target.src), __spreadValues({
        cwd: config.root,
        ignoreInitial: true
      }, watch.options));
      watcher.on("add", async (path2) => {
        config.logger.info(formatConsole(`${pc2.green("detected new file")} ${path2}`), {
          timestamp: true
        });
      ...
      if (watch.reloadPageOnChange) {
        watcher.on("change", (path2) => {
          config.logger.info(formatConsole(`${pc2.green("file changed")} ${path2}`), {
            timestamp: true
          });
          reloadPage();
        });
        watcher.on("unlink", (path2) => {
          config.logger.info(formatConsole(`${pc2.green("file deleted")} ${path2}`), {
            timestamp: true
          });
          reloadPage();
        });
